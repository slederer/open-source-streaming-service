...
<p>That is it. No <code>Stripe-Signature</code> header. A real Stripe event arrives with a header that looks like <code>t=1234567890,v1=hexdigest...</code>, computed by Stripe using a secret you set up when you registered the webhook endpoint. If your server skips the signature check, it has no way to know the event came from Stripe versus from us, versus from anyone with curl.</p>
...
<p>The variant that hits hardest: apps that key off <code>customer_email</code> in the event payload. The attacker doesn't even need a session, just the URL. Stuff their email in the fake event, hit send, get pro.</p>
...
<ol>
<li>Build the integration locally using <code>stripe listen --forward-to localhost</code>. The Stripe CLI strips the signature for local dev, so your local code path doesn't need to verify.</li>
<li>Deploy to production. The signature check is "TODO before launch".</li>
<li>It works. Real Stripe events come through. Customers pay, accounts upgrade. The signature check stays TODO.</li>
<li>Six months later, no one remembers the TODO.</li>
</ol>
...
<pre style="background:#0a0e17;color:#e5e7eb;padding:14px 16px;border-radius:8px;font-size:13px;overflow-x:auto;"><code>const sig = req.headers['stripe-signature'];
let event;
try {
...
<li>Read the <strong>raw</strong> request body, not the parsed JSON. Most frameworks parse JSON by default, which destroys the signature. You typically need to register a special body-reader for the webhook route.</li>
...
<p>The middle step is where most people trip up. Express's <code>express.json()</code> middleware will eat the raw body before your handler sees it. Use <code>express.raw({type: 'application/json'})</code> just on the webhook route. FastAPI users: read <code>await request.body()</code> directly, not <code>request.json()</code>.</p>
